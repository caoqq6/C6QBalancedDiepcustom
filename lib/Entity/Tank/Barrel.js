...
const Jailbreaker_1 = __importDefault(require("./Projectile/Jailbreaker"));
const Object_1 = __importDefault(require("../Object"));
const TankBody_1 = __importDefault(require("./TankBody"));
const LauncherRocket_1 = __importDefault(require("./Projectile/LauncherRocket"));
...
const shotpellet_1 = __importDefault(require("./Projectile/shotpellet"));
const shotpellet2_1 = __importDefault(require("./Projectile/shotpellet2"));
const shotpellet3_1 = __importDefault(require("./Projectile/shotpellet3"));
const shotgun3_1 = __importDefault(require("./Projectile/shotgun3"));
const cshotgun_1 = __importDefault(require("./Projectile/cshotgun"));
const pshotgun_1 = __importDefault(require("./Projectile/pshotgun"));
class ShootCycle {
    constructor(barrel) {
    ...
    shoot() {
        ...
        switch (this.definition.bullet.type) {
            ...
            case "croc":
                ...
                break;
            case "jailbreak":
                new Jailbreaker_1.default(this, this.tank, tankDefinition, angle);
                break;
            case "LauncherRocket":
                new LauncherRocket_1.default(this, this.tank, tankDefinition, angle);
                break;
            case 'ShotPellet': {
                for (let i = 0; i < 4; ++i) {
                    const scatterAngle = (Math.PI / 180) * this.definition.bullet.scatterRate * (Math.random() - .5) * 10;
                    const bullet = new shotpellet_1.default(this, this.tank, tankDefinition, this.definition.angle + scatterAngle + this.tank.positionData.values.angle);
                }
                break;
            }
            case 'Shotgun3': {
                for (let i = 0; i < 3; ++i) {
                    const scatterAngle = (Math.PI / 180) * this.definition.bullet.scatterRate * (Math.random() - .5) * 10;
                    const bullet = new shotgun3_1.default(this, this.tank, tankDefinition, this.definition.angle + scatterAngle + this.tank.positionData.values.angle);
                }
                break;
            }
            case 'Cshotgun': {
                for (let i = 0; i < 5; ++i) {
                    const scatterAngle = (Math.PI / 180) * this.definition.bullet.scatterRate * (Math.random() - .5) * 10;
                    const bullet = new cshotgun_1.default(this, this.tank, tankDefinition, this.definition.angle + scatterAngle + this.tank.positionData.values.angle);
                }
                break;
            }
            case 'Pshotgun': {
                for (let i = 0; i < 28; ++i) {
                    const scatterAngle = (Math.PI / 180) * this.definition.bullet.scatterRate * (Math.random() - .5) * 10;
                    const bullet = new pshotgun_1.default(this, this.tank, tankDefinition, this.definition.angle + scatterAngle + this.tank.positionData.values.angle);
                }
                break;
            }
            case 'ShotPellet2': {
                for (let i = 0; i < 3; ++i) {
                    const scatterAngle = (Math.PI / 180) * this.definition.bullet.scatterRate * (Math.random() - .5) * 10;
                    const bullet = new shotpellet2_1.default(this, this.tank, tankDefinition, this.definition.angle + scatterAngle + this.tank.positionData.values.angle);
                }
                break;
            }
            case 'ShotPellet3': {
                for (let i = 0; i < 3; ++i) {
                    const scatterAngle = (Math.PI / 180) * this.definition.bullet.scatterRate * (Math.random() - .5) * 10;
                    const bullet = new shotpellet3_1.default(this, this.tank, tankDefinition, this.definition.angle + scatterAngle + this.tank.positionData.values.angle);
                }
                break;
            }
            default:
