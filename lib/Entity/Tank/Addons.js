...
const AutosmasherTurret_1 = __importDefault(require("./AutosmasherTurret"));
...
class Addon {
    ...
    createAutoTurrets(count, turret) {
        ...
        for (let i = 0; i < count; ++i) {
            const base = new AutoTurret_1.default(rotator, turret);
...
const AutoTurretMiniSniperDefinition = {
    angle: 0,
    offset: 0,
    size: 65,
    width: 42 * 0.7,
...
class AutoSmasherAddon extends Addon {
    constructor(owner) {
        super(owner);
        this.createGuard(6, 1.15, 0, .1);
        new AutosmasherTurret_1.default(owner);
    }
}
